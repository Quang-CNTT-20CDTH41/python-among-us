...
def layout_impostors(impostors_to_layout):
    number_of_gaps = len(impostors_to_layout) + 1 #số lượng khoảng cách trên màn hình
    gap_size = WIDTH / number_of_gaps
    random.shuffle(impostors_to_layout) # đảo lộn ví trí actor trên layout
    for index, impostor in enumerate(impostors_to_layout): # bộ đếm đặt số 
        new_x_pos = (index + 1)*gap_size # vị trí mới trên trục x
